...
*浏览器内实际运行画面。巨龙传说现为 AWP 默认皮肤；本页既有截图保留当时版本的界面、资源数量与装备清单。*
...
| **两个阵营，分别搭配** | 默认 CS2 SAS / Phoenix 探员；可另选艾娃 / 迈阿密达里尔，真实模型与动作在对局中同步。 |
...
| **先投道具，再进点** | 高爆、闪光、烟雾已可投掷；服务器计算碰撞、爆炸、致盲与烟雾遮挡，详见下方原型边界。 |
...
**道具仍是第一版原型。** 高爆手雷最多 1 枚、闪光弹 2 枚、烟雾弹 1 枚，总计最多 4 枚。烟雾持续 18 秒，以球形区域阻挡机器人视线，并以有限数量的云片和进入烟雾时的屏幕遮罩呈现；闪光按距离、朝向和墙体遮挡计算。暂未加入燃烧弹、诱饵弹、低抛、Source 2 响应式体积烟雾、子弹吹散烟雾、丢枪与退款。
...
- 已修复角色移除时的 GPU 资源泄漏、音频节点释放问题，以及特定异常网络消息可导致服务端退出的问题。本次浏览器短测运行 132 秒、记录 67 次采样与 21 次死亡，0 错误，未观察到堆内存增长。用户报告的整个浏览器关闭仍未定位，短测不能证明所有设备的长时间稳定性；上一版 671 秒测试与本次验证分别记录在[稳定性说明](docs/STABILITY.md)。
