...
export function mountOfflineMenu(){
  ...
  const settingsKeys=['dust2.cs-settings.v1','dust2.quality.v2','dust2.brightness','dust2.controls.v1','dust2.touch.v1','dust2.touch-layout.v1','dust2.offline-fps','dust2.fullscreen.v1','dust2.skins.v1','dust2.skins.v2','dust2.agents.v1','dust2.name','dust2.volume','dust2.music-kit','dust2.music-volume'];
  ...
  $('preferences-file').onchange=async e=>{try{const file=e.target.files[0];if(!file)return;if(file.size>262144)throw Error('备份文件过大');const data=JSON.parse(await file.text());if(data.version!==1||!data.preferences||typeof data.preferences!=='object')throw Error('无效的设置备份');for(const key of settingsKeys){const value=data.preferences[key];if(typeof value==='string'&&value.length<32768)preferences.setItem(key,value);}$('cache-status').textContent='设置已恢复，重新打开游戏后生效。';}catch(error){$('cache-status').textContent='恢复失败：'+error.message;}finally{e.target.value='';}};
  async function refresh(){if(OFFLINE){$('cache-size').textContent='完整资源已随安装包保存';$('cache-count').textContent='地图 · 皮肤 · 探员 · 音乐';$('cache-persistence').textContent='直接读取安装目录，无需重复缓存。设置独立保存，可导出备份。';return;}try{const state=await getAssetCacheStats();$('cache-size').textContent=`${(state.bytes/1048576).toFixed(1)} MB 已保存`;$('cache-count').textContent=`基础资源 ${state.baseCount} / ${state.totalCount} 个${state.complete?' · 已完整保存':''}`;$('cache-persistence').textContent=state.persisted?'浏览器已授予持久存储。':'点击保存时将申请持久存储；若浏览器未授予，缓存仍可复用，但可能被浏览器回收。';if(!state.supported)$('cache-persistence').textContent='此浏览器暂不支持持久资源缓存。';}catch(e){$('cache-status').textContent=e.message;}}
  ...
  if(OFFLINE){for(const id of ['save-base-assets','install-game','clear-cache'])$(id).hidden=true;modal.querySelector('.config-card>p').textContent='离线版本的完整资源已在本机。更换皮肤、探员和音乐不需要联网。';modal.querySelector('small').textContent='设置自动保存；卸载或清除应用数据之前，请导出设置备份。';}
