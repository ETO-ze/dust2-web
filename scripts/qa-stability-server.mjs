...
function setupMovement(room,kind){
 ...
 room.respawn(p);
 const position=kind==='short'?{x:8,y:2.5,z:-47,yaw:-.25,pitch:-.12}:kind==='ramp'?{x:35,y:.5,z:-55,yaw:.1,pitch:-.08}:kind==='window'?{x:-29.5,y:2.2,z:-68,yaw:Math.PI/2,pitch:.05}:kind==='scaffold'?{x:-30,y:2.2,z:-68,yaw:-Math.PI/2,pitch:.1}:kind==='sky'?{...MAP.sites.A,yaw:-.45,pitch:.43}:kind==='flat'?movementFlat:kind==='ledge'?probe.ledge:probe.lower;
...
const control=createServer((req,res)=>{
  ...
  if(/^\/qa\/movement\/(flat|ledge|jump|sky|window|scaffold|short|ramp)$/.test(req.url)){
