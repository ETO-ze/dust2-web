...
当前锁清单为 [`config/assets-lock.json`](../config/assets-lock.json)：**1,569 个文件，465,122,552 字节，约 443.6 MiB**，版本 `f7ae61faeff80fda`。它包含桌面与手机两档地图纹理、共享碰撞与动画、默认及可选武器和探员、声音、音乐盒、界面图像和版本清单。服务器本地恢复整个清单；玩家浏览器只下载设备对应的基础档，可选内容在装备时下载。
