...
if(!args.includes('--run')){console.log(JSON.stringify({networkRequests:0,plan:'Verify changed public client/assets against packaged SHA-256',release:release.release,files:files.length,bytes:files.reduce((n,f)=>n+f.bytes,0)},null,2));}
 ...
 report.finishedAt=new Date().toISOString();report.ok=true;await fs.writeFile(output,JSON.stringify(report,null,2));console.log(JSON.stringify({ok:true,release:report.release,files:report.files.length,bytes:report.files.reduce((n,f)=>n+f.bytes,0)}));
}
